...
class BitsetLite {
	...
	/// Count of significant bits in the last word. Zero means the last word is fully used
	static constexpr size_t last_word_bits = BitCount % word_bits;

	/// Mask for the significant bits of the last word. Used in all()/any() and to_ulong()/to_ullong()
	static constexpr Word last_word_mask = last_word_bits == 0 ? all_ones_word : (Word{1} << last_word_bits) - 1;
...
public:
	...
	constexpr BitsetLite(Word value) noexcept : array_() {
		...
		else if constexpr (retain_mask_needed) {
	...
	constexpr bool all() const noexcept {
		...
		for (size_t i = 0; i + 1 < word_count; ++i) {
			if (array_[i] != all_ones_word) {
				return false;
			}
		}
		return (array_[word_count - 1] & last_word_mask) == last_word_mask;
	}
	...
	constexpr bool any() const noexcept {
		...
		for (size_t i = 0; i + 1 < word_count; ++i) {
			if (array_[i] != all_zeros_word) {
				return true;
			}
		}
		return (array_[word_count - 1] & last_word_mask) != all_zeros_word;
	}
	 ...
	 * @return New bitset
	 */
	constexpr BitsetLite operator~() const noexcept {
		BitsetLite tmp;
